...
(runApiTests ? describe : describe.skip)('FAQ Auto-Complete API', () => {
  test.each(testCases)('returns expected suggestions for %s', async ({ query, expectedTerms, fuzzyTest }) => {
    const response = await fetch(`${BASE_URL}/api/faq/autocomplete?q=${encodeURIComponent(query)}&limit=10`, {
    ...
    const suggestions = data.suggestions.map(s => s.text.toLowerCase())
    expectedTerms?.forEach(term => {
      expect(suggestions.some(s => s.includes(term.toLowerCase()))).toBe(true)
    })
  })
})
