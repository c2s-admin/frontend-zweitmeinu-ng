...
const categorySlugs = ['zweitmeinung-kardiologie', 'zweitmeinung-onkologie', 'allgemeine-fragen-zur-zweitmeinung'];

(runApiTests ? describe : describe.skip)('FAQ Category Relations', () => {
  test('FAQs include populated categories', async () => {
    const response = await fetch(`${BASE_URL}/faqs?populate=category&pagination[limit]=10`, {
  ...
  test.each(categorySlugs)('filtering by category %s', async slug => {
    const response = await fetch(`${BASE_URL}/faqs?filters[category][slug][$eq]=${slug}&populate=category&pagination[limit]=5`, {
      headers: { 'Content-Type': 'application/json' }
    })
